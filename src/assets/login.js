...
class Login extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.requiredCode =
      attrCode && String(attrCode).trim() !== ""
        ? String(attrCode).trim()
        : LOGIN_CODE;
    this.html = `<style>
      /****** LOGIN MODAL ******/
      #login-modal {
    ...
    <div class="modal fade" id="login-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
      <div class="modal-dialog">
        <div class="loginmodal-container">
          ...
          <p>The password is <code>${this.requiredCode}</code></p>
          <form>
    ...
    this.shadowRoot.querySelector("form").addEventListener("submit", (e) => {
      e.preventDefault();
      const inputVal = this.shadowRoot
        .querySelector("input[name=pass]")
        .value.trim();
      if (inputVal === this.requiredCode) {
...
window.addEventListener("load", () => {
  if (!localStorage.getItem("iform-totally-logged-in")) {
    ...
    const el = document.createElement("iform-login");
    el.setAttribute("code", LOGIN_CODE);
    div.appendChild(el);
